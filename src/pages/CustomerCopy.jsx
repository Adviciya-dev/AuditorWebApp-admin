...
const FileUploadPopup = ({ isOpen,setIsOpen,uploadingFile,setUploadingFile,onNext }) => {
  ...
  const { uploadSingleFileToS3, uploadMultipleFilesToS3 } = useUploadToS3();
    ...
    if (newFiles.length > 0) {
      ...
      try {
        ...
        const uploadPromises = newFileObjects?.map((fileObj, index) => {
          ...
          // Upload single file with its own progress handler
          return uploadSingleFileToS3(fileObj.file, handleProgress)
            .then((response) => {
              setUploadingFile((prevFiles) =>
                prevFiles.map((file) => {
                  if (file.name === fileObj.name) {
